...
if (process.env.NODE_ENV !== 'production') {
  (function() {
    ...
    function processBinaryChunkFromResponse(fetchReponseBody) {
      var response = createResponse$1();
      return new Promise((resolve, reject) => {
        startReadingFromStream(response, fetchReponseBody)
          .then(() => {
            resolve(response.readRoot());
          })
          .catch(reject);
      });
    }

    exports.processBinaryChunkFromResponse = processBinaryChunkFromResponse;
    ...
    function createFromFetch(promiseForResponse) {
      ...
      return new Promise((resolve, reject) => {
        promiseForResponse.then(
          function(r) {
            // r.text().then((r) => { console.log("Parse text", r) })
            // debugger
            startReadingFromStream(response, r.body).then(() => {
              resolve(response);
            });
          },
          function(e) {
